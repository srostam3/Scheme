...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if isinstance(symbol, Pair):
            return scheme_eval(symbol, self)
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        if operands:
            new_op = Pair.map(operands, lambda oper: scheme_eval(oper, env))
            return scheme_apply(self,new_op, env)
        else:
            return scheme_apply(self, operands, env)
        #return Pair.map(operands, lambda oper_input: scheme_apply(self, oper_input, env))
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        answer = scheme_eval(expressions.second.first, env)
        env.define(target, answer)
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        answer = LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first, answer)
        return target.first
        # END PROBLEM 10
    else:
